    ...
    while (true) {
        ...
        if (UserOption == 2) {
            ...
            for (int i = 0; i < service.getSizeOfArray(); i++)
                cout <<"Position: "<<i<<" "<<"Title: "<< movies[i].getTitle() << " | Genre: " << movies[i].getGenre() << " | Year: " << movies[i].getYear() << " | Likes: "
                     << movies[i].getLikes() << " | Trailer: " << movies[i].getTrailer() << "\n";
            cout << "Position of the movie you want to delete: ";
            cin >> position;
            service.deleteMovie(position);
        }
        if (UserOption == 3) {
            string title, genre, trailer, init_year, init_likes;
            int year, likes;
